  ...
  return (
    <main className={`harness-detail plane-${harness.plane}`}>
        ...
        <div className="detail-layout section-shell">
          ...
          <div className="detail-content">
            ...
            <section id="interfaces"><div className="section-number">02 / INTEGRATION POINTS</div><div className="interface-list">{related.map((item, index) => {
              ...
              return <a key={`${item.h}-${index}`} href={`/blueprint/${item.h}`}><span>{String(item.h).padStart(2, '0')}</span><div><b>{target?.name}</b><p>{item.why}</p></div><span aria-hidden="true">↗</span></a>;
            })}</div></section>
      ...
      <nav className="harness-pagination section-shell" aria-label="Harness pages">
        {previous ? <a href={`/blueprint/${previous.n}`}><span>Previous</span>{String(previous.n).padStart(2, '0')} · {previous.name}</a> : <span />}
        {next ? <a href={`/blueprint/${next.n}`}><span>Next</span>{String(next.n).padStart(2, '0')} · {next.name}</a> : <a href="/blueprint"><span>Next</span>Return to blueprint</a>}
      </nav>
      <SiteFooter />
    </main>
  );
