...
Without explicit engine/mail configuration, public references and brief editing work, but generation, verification and email correctly report unavailable. The local service health reports configuration availability, not a successful model invocation or inbox delivery.
...
## Separate live activation gates — NOT RUN

1. **Inference identity:** provision or identify a dedicated Planeon credential on the existing engine, scoped to approved self-hosted models. Supply the engine's OpenAI-compatible `/v1` base, model and credential only to the local service. Verify engine-side provider fallback and prompt/body logging are disabled. Set `STUDIO_INFERENCE_IDENTITY=planeon` and `STUDIO_SELF_HOSTED_APPROVAL=self-hosted:<exact-model>`. This operator attestation is not automatic proof of the engine's configuration. Test structured JSON support and model identity before exposing it publicly. Never borrow another company's account or key.
2. **Private environment:** use `services/journey-studio/.env.example` as a field reference. Keep the actual environment file in the private data directory, mode 0600, not Git or a sync folder. The service reads process environment; it does not auto-load this template. A private environment can be loaded with Node's `--env-file` flag when launching `src/supervisor.ts` through `--import tsx`. Never put these values in `VITE_*` variables or client bundles.
