...
`Send answers` is available above and below the question list; the upper action bar stays visible during normal-height scrolling and becomes static on short viewports. It can send partial answers through the existing `/api/studio/assistant` interface. Receipt appears only after a successful response for the unchanged brief; edits enable `Send updated answers`, failures retain the entries, and completing the set exposes `Continue to brief`. Sending does not retain the public draft on the server.

The service reviews the matched answers deterministically. It keeps original unanswered questions visible and does **not** ask the model to generate another clarification list. This prevents the reported paraphrasing loop by changing the interaction contract, not by claiming a prompt or similarity heuristic can reliably recognize every paraphrase. Additional gaps belong in Missing information; completion of a round is not a completeness assessment.
