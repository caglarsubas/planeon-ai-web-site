...
Journey now uses up to five question rounds with one labelled answer field per question. Stable `q1`–`q25` identities belong to the original questions, not subsequent model wording. Optional round labels preserve compatibility with old `q1`–`q5` drafts. Visitors can supply an answer, label it an assumption, or explicitly defer it. Answers can be edited; any edit invalidates confirmation and a pending proposal. The confirmed brief carries these pairs separately from the six-turn chat window.

`Send answers` is available above and below the question list; the upper action bar stays visible during normal-height scrolling and becomes static on short viewports. It can send partial answers through the existing `/api/studio/assistant` interface. Every submission reaches the approved inference model. Receipt appears only after a successful response for the unchanged brief; edits enable `Send updated answers`, and failures retain the entries and round. New questions are prominent; earlier answers remain editable in an expansion. Sending does not retain the public draft on the server.

The initial question set is round one. Successful follow-up question sets advance through round five; retries do not advance the counter. The model receives the whole matched ledger and is instructed to ask only essential new decisions, not paraphrase answered or deferred questions. It may finish earlier when ready to draft. After round-five answers, the application enforces zero further questions even if the model asks for more, proposing a brief with labelled rough assumptions and unresolved sensitive decisions. Exact repeated question sets enter the bounded repair path rather than falsely reporting readiness; semantic paraphrase prevention still depends on model quality, with the five-round cap as a deterministic limit. The former deterministic answer-acknowledgement bypass has been removed.

Continuing to recipe generation requires explicit application/review of the proposed brief, not merely filling every field. Pending unanswered entries become retained open decisions at completion, never invented visitor answers. Model additions cannot become supplied facts. Access rights, approval authority and regulatory uncertainty are never established by a rough planning assumption. Terminal brief edits may be resubmitted for review but cannot reopen questioning beyond round five. All approval and document-delivery checks remain unchanged.
...
Open `http://localhost:3001/journey?mode=design`. The service listens **only on 127.0.0.1:4320**. The local bridge shares this default; 4318 is reserved for OTLP and collided with the inference stack's collector. Restart the service and website preview after upgrading. If overriding `STUDIO_PORT`, also set the website's server-only `STUDIO_SERVICE_URL` to the matching loopback address. Do not stop or reconfigure another project's collector. Its foreground supervisor retries crashes with exponential backoff, stops after five repeated failures, and installs no daemon. Keep that terminal running; quitting it stops the feature. SIGTERM/SIGINT drain HTTP and current worker work. Hard interruption leaves SQLite checkpoints for restart recovery. Sleep/disconnect means genuine unavailability, not a cloud fallback.
...
## Visitor path: draft walkthrough and output files

The first validated proposal immediately renders in the shared harness-onion, paused at 0.75×. It also supports Sequence, Flat waterfall and Tree waterfall. Selected-step evidence is restricted to that step's feature and harness references; selecting a named harness opens its reference context. Approval waits and non-task activity remain explicit pauses. Switching to Explore or inspecting a pending revision pauses the selected canvas without replacing its version.

“Use this draft” selects the proposed version; it is not reviewer approval. Later revisions have a separate collapsed onion preview and a change comparison. Only an explicit “Apply this revision” changes the selected draft. Public drafts are still tab-local.

“Get PDF, Markdown & JSON” leads to the engineering-pack section. The visitor verifies a company inbox, supplies the mandatory profile/intended use and consent, and requests preparation of the selected confirmed snapshot. Caglar reviews the prepared exact bytes. After approval the visitor signs into `/journey/requests` with the same email, selects the request, and downloads each format. My requests links from the draft open in a new tab to preserve the unsaved public draft. The existing owner/version/approval/retention checks remain authoritative; an unsigned preview cannot request or download a pack.

When verification email is unavailable, the interface explains the blocker before code entry. No sign-in bypass, browser-generated export or public file URL is supplied. Existing authenticated owners can still reach approved files while the service is online, even if email notification delivery failed. Email activation and real mail testing remain separate release steps.
